...
func setupIntegration(t *testing.T) (*sqlStore, func()) {
	t.Helper()
...
func TestIntegrationUserFlow(t *testing.T) {
	...
	u, err := svc.Create(ctx, CreateUserInput{Pseudo: "alice", Ville: "Paris"})
	if err != nil {
	...
	if err := svc.ReplaceSkills(ctx, u.ID, u.ID, []Skill{{Nom: "Jardinage", Niveau: "expert"}}); err != nil {
	...
	updated, err := svc.Update(ctx, u.ID, u.ID, UpdateUserInput{Pseudo: "alice2", Bio: "bio"})
...
func TestIntegrationServiceAndExchangeFlow(t *testing.T) {
	...
	alice, _ := userSvc.Create(ctx, CreateUserInput{Pseudo: "alice"})
	bob, _ := userSvc.Create(ctx, CreateUserInput{Pseudo: "bob"})
	_ = userSvc.ReplaceSkills(ctx, alice.ID, alice.ID, []Skill{{Nom: "Jardinage", Niveau: "expert"}})

	svc, err := serviceSvc.Create(ctx, alice.ID, CreateServiceInput{
	...
	services, err := serviceSvc.List(ctx, ListServicesInput{Categorie: "Jardinage", Ville: "Paris", Search: "Tont"})
	if err != nil || len(services) != 1 {
		t.Fatalf("list services: %v, count=%d", err, len(services))
	}

	ex, err := exchangeSvc.Create(ctx, bob.ID, CreateExchangeInput{ServiceID: svc.ID})
	if err != nil {
		t.Fatalf("create exchange: %v", err)
	}
	if _, err := exchangeSvc.Create(ctx, bob.ID, CreateExchangeInput{ServiceID: svc.ID}); err == nil {
...
func TestIntegrationReviewAndStats(t *testing.T) {
	...
	svc, _ := serviceSvc.Create(ctx, alice.ID, CreateServiceInput{
		Titre: "Tonte", Categorie: "Jardinage", DureeMinutes: 60, Credits: 2,
	})
	ex, _ := exchangeSvc.Create(ctx, bob.ID, CreateExchangeInput{ServiceID: svc.ID})
	_, _ = exchangeSvc.Accept(ctx, alice.ID, ex.ID)
	_, _ = exchangeSvc.Complete(ctx, alice.ID, ex.ID)

	if _, err := reviewSvc.Create(ctx, bob.ID, ex.ID, CreateReviewInput{Note: 5}); err != nil {
		t.Fatalf("create review: %v", err)
	}
	if _, err := reviewSvc.Create(ctx, bob.ID, ex.ID, CreateReviewInput{Note: 3}); err == nil {
...
func TestIntegrationListExchanges(t *testing.T) {
	...
	list, err := exchangeSvc.List(ctx, alice.ID, StatusPending)
...
func TestIntegrationUpdateAndDeleteService(t *testing.T) {
	...
	updated, err := serviceSvc.Update(ctx, alice.ID, svc.ID, UpdateServiceInput{
