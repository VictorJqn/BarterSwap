...
func (s *Store) GetServiceByID(ctx context.Context, id int) (domain.Service, error) {
	var (
		svc       domain.Service
		createdAt time.Time
	)
...
func (s *Store) ListServices(ctx context.Context, filter domain.ServiceFilter) ([]domain.Service, error) {
	var (
		conds  []string
		args   []any
		argIdx = 1
...
func (s *Store) UpdateService(ctx context.Context, svc domain.Service) (domain.Service, error) {
	...
	if n, _ := res.RowsAffected(); n == 0 {
		return domain.Service{}, fmt.Errorf("%w: service %d", apperr.ErrNotFound, svc.ID)
	}
