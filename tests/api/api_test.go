...
func TestAPICreateUser(t *testing.T) {
	...
	user := decodeBody[domain.User](t, rec)
	if user.Pseudo != "alice" || user.CreditBalance != domain.WelcomeCredits {
...
func TestAPIServiceWithoutSkill(t *testing.T) {
	store := mock.New()
	store.SeedUser(domain.User{ID: 1, Pseudo: "bob"}, 10, nil)
...
func TestAPIFullExchangeFlow(t *testing.T) {
	...
	service := decodeBody[domain.Service](t, svcRec)
	...
	ex := decodeBody[domain.Exchange](t, exRec)
	...
	bob := decodeBody[domain.User](t, bobRec)
	...
	alice := decodeBody[domain.User](t, aliceRec)
...
func TestAPIUserStats(t *testing.T) {
	...
	stats := decodeBody[domain.UserStats](t, rec)
...
func TestAPIAdditionalEndpoints(t *testing.T) {
	...
	store.SeedUser(domain.User{ID: 1, Pseudo: "alice"}, 10, []domain.Skill{{Nom: "Jardinage", Niveau: "expert"}})
	store.SeedUser(domain.User{ID: 2, Pseudo: "bob"}, 10, nil)
	store.SeedService(domain.Service{ID: 1, ProviderID: 1, Titre: "Tonte", Categorie: "Jardinage", DureeMinutes: 60, Credits: 2, Actif: true, Ville: "Paris"})
	store.SeedExchange(domain.Exchange{ID: 1, ServiceID: 1, RequesterID: 2, OwnerID: 1, Status: domain.StatusPending})
	...
	tests := []struct {
		...
		{"replace skills", http.MethodPut, "/api/users/1/skills", []domain.Skill{{Nom: "Jardinage", Niveau: "expert"}}, "1", http.StatusOK},
...
func TestAPICancelExchange(t *testing.T) {
	...
	store.SeedUser(domain.User{ID: 2, Pseudo: "bob"}, 10, nil)
	store.SeedService(domain.Service{ID: 1, ProviderID: 1, Titre: "Tonte", Categorie: "Jardinage", DureeMinutes: 60, Credits: 2, Actif: true})
	store.SeedExchange(domain.Exchange{ID: 1, ServiceID: 1, RequesterID: 2, OwnerID: 1, Status: domain.StatusAccepted})
	store.SetCreditBalance(2, 8)
	handler := newTestAPI(store)
	...
	ex := decodeBody[domain.Exchange](t, rec)
	if ex.Status != domain.StatusCancelled {
...
func TestAPIListExchangesWithStatus(t *testing.T) {
	...
	exchanges := decodeBody[[]domain.Exchange](t, rec)
	if len(exchanges) != 1 || exchanges[0].Status != domain.StatusPending {
...
func TestAPIForbiddenAndNotFound(t *testing.T) {
	...
	tests := []struct {
		...
		{"replace skills forbidden", http.MethodPut, "/api/users/1/skills", []domain.Skill{{Nom: "Jardinage", Niveau: "expert"}}, "2", http.StatusForbidden},
...
func TestMiddlewareRecoveryAndCORS(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	httpapi.WithMiddleware(panicHandler).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/", nil))
	if rec.Code != http.StatusInternalServerError {
		t.Fatalf("recovery status = %d", rec.Code)
	}

	corsRec := httptest.NewRecorder()
	httpapi.WithMiddleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
